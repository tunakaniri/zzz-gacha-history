...
export async function onSend(urlVar, jsonVar) {
    ...
    const base_url='https://public-operation-nap-sg.hoyoverse.com/common/gacha_record/api/getGachaLog',size=20,api_url = ref(''), end_id = ref('');
    ...
    api_url.value = base_url + '?authkey=' + urlVar.authkey.value
    for (loop = 0; ; loop++) {
        if (len === size || loop === 0) {
            await sleep(500);
            await getGachaData();
